...
export function CertificateCard({ cert, index: i, initialCount }: Props) {
  return (
    <motion.div
      ...
      className="group relative rounded-2xl bg-white dark:bg-[#111] border border-gray-200 dark:border-[#1e1e1e] hover:border-[#c8956a]/40 transition-colors duration-400 overflow-hidden"
      whileHover={{ y: -4 }}
    >
      ...
      {cert.image ? (
        ...
        <div className="w-full h-40 bg-gradient-to-br from-[#c8956a]/10 to-white dark:to-[#111] flex items-center justify-center border-b border-gray-200 dark:border-[#1e1e1e]">
      ...
      <div className="relative z-10 p-6">
        <div className="mb-4 min-h-[4.5rem]">
          <h3 className="font-bold text-gray-900 dark:text-white text-sm leading-snug mb-1 group-hover:text-[#c8956a] transition-colors duration-300 line-clamp-3">
            {cert.title}
          </h3>
          <p className="text-xs text-gray-500 font-semibold">{cert.institution}</p>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-4 p-4 rounded-xl bg-[#faf7f3] dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#151515]">
          <div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-900 dark:text-white font-bold">{cert.hours}</p>
          </div>
        </div>
        ...
        <a
          ...
          className="flex items-center justify-between p-3 rounded-xl bg-[#faf7f3] dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#151515] group-hover:border-[#c8956a]/20 transition-colors duration-300"
        >
          <div>
            <p className="text-xs text-gray-600 mb-0.5">ID Credencial</p>
            <p className="text-xs text-gray-600 dark:text-gray-400 font-mono truncate max-w-[180px]" title={cert.credential}>{cert.credential}</p>
          </div>
