...
export function Feedbacks() {
  ...
  return (
    <section id="feedbacks" className="py-16 md:py-32 px-6 relative overflow-hidden bg-[#faf7f3] dark:bg-[#060606]">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-10 md:mb-16 text-left md:text-center">
          ...
          <motion.h2 {...fadeUp(0.1)} className="text-4xl md:text-7xl font-extrabold mb-6 tracking-tight">
            <span className="bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 bg-clip-text text-transparent">O que clientes </span>
            <br />
            <span className="bg-gradient-to-r from-[#c8956a] via-[#d4a882] to-[#e0bc9a] bg-clip-text text-transparent">dizem sobre mim</span>
          </motion.h2>
          <motion.p {...fadeUp(0.2)} className="text-lg md:text-xl text-gray-600 dark:text-gray-400 md:max-w-3xl md:mx-auto leading-relaxed">
          ...
          <div ref={containerRef} className="overflow-hidden">
            <motion.div className="flex items-stretch" style={{ x: xMV, gap: `${GAP}px` }}>
              {track.map((t, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center justify-between text-center p-5 rounded-2xl bg-white dark:bg-[#111] border border-gray-200 dark:border-[#1e1e1e] hover:border-[#c8956a]/30 transition-colors duration-300 flex-shrink-0"
                  style={{ width: cardW > 0 ? cardW : `calc((100% - ${GAP * (cols - 1)}px) / ${cols})`, minHeight: '220px' }}
                >
                  {/* Stars + Text agrupados */}
                  <div className="flex flex-col items-center gap-3">
                    <StarRating />
                    <p className="text-gray-600 dark:text-gray-400 text-xs leading-relaxed italic line-clamp-5">"{t.text}"</p>
                  </div>

                  {/* Author sempre na base */}
                  <div className="mt-4 pt-3 border-t border-gray-200 dark:border-[#1a1a1a] w-full flex flex-col items-center gap-2">
