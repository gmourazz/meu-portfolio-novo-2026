...
export function Technologies() {
  return (
    <section id="tecnologias" className="py-16 md:py-32 px-6 relative overflow-hidden bg-white dark:bg-[#111]">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-10 md:mb-24 text-left md:text-center">
          ...
          <motion.h2 {...fadeUp(0.1)} className="text-5xl md:text-7xl font-extrabold mb-6 tracking-tight">
            <span className="bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 bg-clip-text text-transparent">Ferramentas que </span>
            <br />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mb-10 md:mb-20">
          {categories.map((cat, ci) => (
            <motion.div key={cat.label} {...fadeUp(ci * 0.08)} className="p-6 rounded-2xl bg-white dark:bg-[#111] border border-gray-200 dark:border-[#1e1e1e] hover:border-[#c8956a]/30 transition-colors duration-300">
              ...
              <div className="flex flex-col gap-2.5">
                {cat.skills.map((skill, si) => (
                  <motion.div key={skill} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: ci * 0.06 + si * 0.05, duration: 0.4 }} className="flex items-center gap-2.5 group">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors duration-200 font-medium">{skill}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-5 mb-10">
          {highlights.map((h, i) => (
            <motion.div key={h.label} {...fadeUp(i * 0.1)} className="group p-8 rounded-2xl bg-white dark:bg-[#111] border border-gray-200 dark:border-[#1e1e1e] hover:border-[#c8956a]/40 transition-colors duration-500 overflow-hidden relative" whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
              ...
              <div className="relative z-10">
                ...
                <h4 className="font-bold text-gray-900 dark:text-white mb-1">{h.label}</h4>
        ...
        <motion.div {...fadeUp(0.2)} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {extras.map((e, i) => (
            <motion.div key={e.label} {...fadeUp(i * 0.05)} className="p-5 rounded-xl bg-white dark:bg-[#111] border border-gray-200 dark:border-[#1e1e1e] hover:border-[#c8956a]/25 transition-colors duration-300 text-center group">
